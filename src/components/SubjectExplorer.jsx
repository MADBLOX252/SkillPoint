...
import { ChevronRight, Download, BookOpen, FileText, HelpCircle, Book, Link as LinkIcon, Folder } from 'lucide-react';
import { Link, useParams, useNavigate } from 'react-router-dom';
import { CURRICULUM_DATA, SUBJECTS_LIST } from '../constants';
    ...
    return (
      <div className="max-w-4xl mx-auto py-20 px-6">
        <motion.div
          ...
          className="mb-16"
        >
          <Link to="/subjects" className="text-brand-accent hover:underline mb-4 inline-block">← Back to subjects</Link>
  ...
  const renderSectionContent = () => {
    ...
    switch (section) {
      ...
      case 'resources':
        ...
        const grouped = getGroupedItems(rawItems);
  ...
  return (
    <div className="max-w-4xl mx-auto py-20 px-6">
      <motion.div
        ...
        className="mb-16"
      >
        <Link to={`/subjects/${subjectId}`} className="text-brand-accent hover:underline mb-4 inline-block">← Back to {subject.title}</Link>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {item.description && <p className="text-sm text-white/40">{item.description}</p>}
          </div>
        </div>
          ...
          >
            <div className="px-8 pb-8 pt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              {chapter.subChapters.map((sub) => (
                <Link 
                  key={sub.id}
                  to={`/notes/${subjectId}/${chapter.id}/${sub.id}`}
